...
/// <summary>
/// This interface needs to be implemented by the Kinect gesture managers, like KinectGesturesT-class itself
/// </summary>
public interface GestureListenerInterfaceT
{
	/// <summary>
	...
	void CheckForGesture (long userId, ref KinectGesturesT.GestureData gestureData, float timestamp, ref Vector3[] jointsPos, ref bool[] jointsTracked);
}
...
/// </summary>
public class KinectGesturesT : MonoBehaviour, GestureListenerInterfaceT
{
			...
			case Gestures.Rotation:
				switch (gestureData.state)
				{
					/*case 0:  // gesture detection
						...
						{
							SetGestureJoint(ref gestureData, timestamp, leftHandIndex, jointsPos[leftHandIndex]);
						}else if (jointsTracked[rightHandIndex] && jointsTracked[rightShoulderIndex] && // check right hand is straight up 
						  (jointsPos[rightHandIndex].y - jointsPos[rightShoulderIndex].y) > 0.25f &&  // ensure right hand is higher than shoulder 
						  Mathf.Abs(jointsPos[rightHandIndex].z - jointsPos[rightShoulderIndex].z) < 0.45f &&   // ensue hand is vertical straight enough 
						  Mathf.Abs(jointsPos[rightHandIndex].x - jointsPos[rightShoulderIndex].x) < 0.3f &&   // ensue hand is vertical straight enough                             
						  jointsTracked[leftHandIndex] && jointsTracked[leftShoulderIndex] &&  // check left hand is straight flat 
					...
					case 1:  // gesture complete
						...
						if (!isInPose)
                        {
							...
							isInPose = jointsTracked[rightHandIndex] && jointsTracked[rightShoulderIndex] && // check right hand is straight up 
							   (jointsPos[rightHandIndex].y - jointsPos[rightShoulderIndex].y) > 0.25f &&  // ensure right hand is higher than shoulder 
							   Mathf.Abs(jointsPos[rightHandIndex].z - jointsPos[rightShoulderIndex].z) < 0.45f &&   // ensue hand is vertical straight enough 
							   Mathf.Abs(jointsPos[rightHandIndex].x - jointsPos[rightShoulderIndex].x) < 0.3f &&   // ensue hand is vertical straight enough                             
							   jointsTracked[leftHandIndex] && jointsTracked[leftShoulderIndex] &&  // check left hand is straight flat 
