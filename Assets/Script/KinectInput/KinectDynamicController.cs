...
namespace KARV
{
    public class KinectDynamicController : MonoBehaviour
    {
            ...
            if (manager && manager.IsInitialized())
            {
                ...
                if (jointPrefab)
                {
                    ...
                    for (int i = 0; i < joints.Length; i++)
                    {
                        ...
                        JointDeformer joint = joints[i].GetComponent<JointDeformer>();
                        if (joint != null)
                        { 
                            joint.SetJointType((KinectInterop.JointType)i);
                            captureManager.DynamicPositionJoints.Add(joint);
                        }

                        if ((KinectInterop.JointType)i == KinectInterop.JointType.HandLeft)
                            captureManager.leftHand = joint;
                        else if ((KinectInterop.JointType)i == KinectInterop.JointType.HandRight)
                            captureManager.rightHand= joint;
